...
- 내부 클래스의 종류는 변수의 선언위치에 따른 종류와 같다
  - 내부 클래스는 변수를 선언하는 것과 같은 위치에 선언할 수 있으며, 변수의 선언이치에 따라 인스턴스변수, 클래스변수(static변수), 지역변수로 구분되는 것과 같이 내부 클래스도 선언위치에 따라 구분된다
    자바에서는 내부 클래스(Inner Class)라고 불리는 다양한 종류의 클래스를 정의할 수 있습니다. 내부 클래스는 클래스 내부에 다른 클래스를 선언하고 사용하는 기술로, 코드의 구조를 더 모듈화하고 캡슐화할 수 있게 도와줍니다. 다음은 자바에서 사용할 수 있는 내부 클래스의 주요 종류와 특징을 설명한 것입니다.
